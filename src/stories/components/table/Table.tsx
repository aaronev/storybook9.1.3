...
export const Table = ({
  ...
  return (
    <div>
      <h1 class="visual-caption" aria-hidden="true">{caption}</h1>
      <div class={`${selectable ? 'selectable-table': ''} table-container`}>
        <div class="toolbar">
        ...
        <table ref={tableRef} {...props}>
          <caption><span class="visually-hidden">{caption}</span></caption>
          <thead><tr>
            {selectable && <th><div class="visually-hidden">Select Row</div></th>}
          ...
          <tbody>
            {data.map((row: {}) : HTMLElement => {
              ...
              return <tr class={row.status !== "available" ? "disabled": ""}onClick={(e) => { 
                ...
                selectable &&
                  <td>
                    ...
                    onClick={(e) => {
                      ...
                      setSelectAllHint(`${checked.length} items selected`);
                ...
                {
                  tdCells.map(cell => <td className={cell}>
                    <div class={row[cell] === 'available' ? 'available' : ""}>
                      {row[cell] === 'available' && <span class="available-dot" aria-hidden="true">•</span>}
                      <span>{row[cell]}</span>
                      </div>
                    </td>)
                }
